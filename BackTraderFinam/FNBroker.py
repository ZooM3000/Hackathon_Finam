...
class FNBroker(with_metaclass(MetaFNBroker, BrokerBase)):
    ...
    def getposition(self, data):
        ...
        if response.positions == []:
            self.positions[data._name] = Position(0.0, 0.0)
    ...
    def sell(self, owner, data, size, price=None, plimit=None, exectype=None, valid=None, tradeid=0, oco=None, trailamount=None, trailpercent=None, parent=None, transmit=True, **kwargs):
        """Заявка на продажу"""
        #print("SELL")
        order = self.create_order(owner, data, size, price, plimit, exectype, valid, oco, parent, transmit, False, False, **kwargs) #исправил is_buy=False
        self.notifs.append(order.clone())  # Уведомляем брокера о принятии/отклонении зявки на бирже
        return order
    ...
    def get_all_active_positions(self):
        #print('GET_ALL_ACTIVE_POSITIONS')
        ...
        for position in response.positions:  # Пробегаемся по всем активным позициям счета
            ...
            #print('si ', si)
            cross_rate = next(item.cross_rate for item in response.currencies if item.name == position.currency)  # Кол-во рублей за единицу валюты
            #print('cross_rate ', cross_rate)
            price =  position.average_price * cross_rate
            #print('price ', price)
            dataname = position.security_code #self.provider.board_symbol_to_dataname(si.board, si.code)  # исправил название тикера
            #print('dataname ', dataname)
            #print('!!!INSIDE_POSITIONS!!!',position)
            self.positions[dataname] = Position(position.balance, price)  # Сохраняем в списке открытых позиций
        
        #if response.positions == []:
        #    dataname = position.security_code
        #    self.positions[dataname] = Position(0, 0)
    ...
    '''
    def get_alive_order(self) -> Union[Order, None]:
        """Получение установленного ордера на бирже"""
        orders = self.provider.get_stops(self.client_id).stops
        for order in orders:  # Пробегаемся по всем заявкам на бирже
            print('order in broker ', order)
            if order.Alive == True:  # Если нашли совпадение по номеру транзакции
                return order  # то возвращаем заявку BackTrader
        return None  # иначе, ничего не найден
    '''
    ...
    def place_order(self, order: Order):
        ...
        #board = order.info['board']  # исправил Код биржи
        board = 'TQBR' #исправил площадку
        ...
        if order.exectype == Order.Market:  # Рыночная заявка
            ...
            #order.addinfo(transaction_id=response.transaction_id)  # Идентификатор транзакции добавляем в заявку
        elif order.exectype == Order.Stop:  # Стоп заявка испарвил не отправлялась на биржу!
            response = self.provider.new_stop(client_id = self.client_id, security_board = board, security_code = symbol, buy_sell =  buy_sell,
                                              stop_loss = StopLoss(activation_price=order.price, market_price=True,
